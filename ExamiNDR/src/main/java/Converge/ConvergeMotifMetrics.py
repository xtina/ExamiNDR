...
ConvergeMotifMetrics -- Kitchen sink of routines for evaluating motifs on genomes



This is a slight variation on TAMO.MotifMetrics, used by the Converge motif discovery method.
...
for motif metrics.  For a summary of the command-line usage, just type
    ...
    python ConvergeMotifMetrics.py
...
The ConvergeMotifMetrics module performs fast motif evaluation by converting
...
ConvergeMotifMetrics.ProbeSet object (often given the variable name "genome"),
...
% ConvergeMotifMetrics.py GCN4_YPD.fsa -g $TAMOdata/yeast/yesat6k_probes.fsa TGASGCA
...
Genome = ConvergeMotifMetrics.ProbeSet('../yeast/yeast6k_probes.fsa')
...
class ProbeSet:
    ...
    def __init__(self,genome='HUMAN',factor=0.7):
	...
	self.kfactor = 0.11
        ...
        if type(genome) == type({}):
        ...
        elif genome.find('HUMAN') >= 0 and genome.find('_') >= 0 :

            toks = genome.split('_')

            self.fastafile    = ConvergeDataDir+'elongated_probesQC%s.fsa'%(toks[1])

            self.xlatefile    = ConvergeDataDir+'Hu13K_MASTERforben.txt'

            self._loadfiles()

        elif genome=='HUMAN':

            self.fastafile    = ConvergeDataDir+'promoter_seqs_upcase.fsa'
            ...
            fn=ConvergeDataDir+'yeast_alignments_by_probe.pickle'

            FH = open(fn, 'r')

            self.alignments = pickle.load(FH)

    	    FH.close()

            self.speciesDict = {'Scer': 0, 'Spar': 1, 'Smik': 2, 'Sbay': 3, 'Skud': 4, 'Scas': 5, 'Sklu': 6}

            self.w_dict = {'Scer': 1.0, 'Spar': 0.77, 'Smik': 0.67, 'Sbay': 0.61, 'Skud': 0.64, 'Scas': 0.45, 'Sklu': 0.45}

            #self.speciesDict = {'Scer': 0, 'Spar': 1, 'Smik': 2, 'Sbay': 3}

            #self.w_dict = {'Scer': 1.0, 'Spar': 0.77, 'Smik': 0.67, 'Sbay': 0.61}



            self.fastafile    = ConvergeDataDir+'DBGint.fsa'

            self.xlatefile    = ''

            self._loadfiles()

        elif genome=='YEAST_CONSERVED':

            self.fastafile    = ConvergeDataDir+'RGoverlap_1.fsa'

            self.xlatefile    = ''

            self._loadfiles()

        elif genome=='YEAST_2000_UP':

            self.fastafile    = ConvergeDataDir+'utr5_sc_2000.fasta'

            self.xlatefile    = ''

            self._loadfiles()

        elif genome=='YEAST_500_UP':

            self.fastafile    = ConvergeDataDir+'utr5_sc_500.fasta'
            ...
            self.fastafile    = ConvergeDataDir+'SLR14.2_genes.fsa'
            ...
            self.fastafile    = ConvergeDataDir+'All_Martone_bynum.fsa'

            self.fastafile    = ConvergeDataDir+'Chr22_PCR_array.fa'
            ...
            if self.fastafile.find('elongated_probesQC') >= 0: 
    ...
    def Enrichment(self,ambig, NMlist,verbose='',factor=None,omitlist=[]):
        ...
        return self.p_value(self,ambig, NMlist,verbose   ,factor     ,omitlist)
    ...
    def ids_from_file(self,filename,QUIET=''):
        ...
        if re.search('(\.fa|\.fsa|\.fasta)$',filename):
